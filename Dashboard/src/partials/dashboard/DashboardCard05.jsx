...
const [chartData, setChartData] = useState({
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:8000/count_cam/?cam_id=2');
  ...
  return (
    <div className="flex flex-col col-span-full sm:col-span-6 xl:col-span-4 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700">
      <header className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 items-center">
        <img src={Icon} width="26" height="23" alt="Icon 03" />
        <h5 className="mt-3 font-semibold text-slate-800 dark:text-slate-100">Stall - 3 Visitors Count</h5>
      </header>
